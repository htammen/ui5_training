...
sap.ui.define([
	...
	"de/tammenit/ui5/training/model/formatter",
	"sap/m/MessageBox",
	"sap/m/MessageToast",
	"sap/ui/Device"
], function(BaseController, JSONModel, formatter, MessageBox, MessageToast, Device) {
	"use strict";

	return BaseController.extend("de.tammenit.ui5.training.controller.Detail", {
		...
		/**
		 * Eventhandler for selection on ListItem in order table
		 */
		onTableSelectionChange: function(oEvent) {
			var oItem = oEvent.getSource();
			this._showOrderDetail(oItem);
		},
		...
		/**
		 * Shows the selected item on the order detail page
		 * On phones a additional history entry is created
		 * @param {sap.m.ObjectListItem} oItem selected Item
		 * @private
		 */
		_showOrderDetail: function(oItem) {
			var bReplace = !Device.system.phone;
			// navigate to route orderDetail with "pattern": "customers/{id}/{orderid}"
			this.getRouter().navTo("orderDetail", {
				id: encodeURIComponent(this.getView().getBindingContext().getProperty("id")),
				orderid: encodeURIComponent(oItem.getBindingContext().getProperty("id"))
			}, bReplace);
		},
		...
		_onObjectMatched: function(oEvent) {
			...
			this.getModel().metadataLoaded().then(function() {
				var sObjectPath = this.getModel().createKey("customers", oParameter);
